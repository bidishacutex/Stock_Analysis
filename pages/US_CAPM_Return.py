...
# Reset index if previously set to Date or something else
stocks_df.reset_index(inplace=True)
SP500.reset_index(inplace=True)

# Rename SP500 column for clarity
SP500.columns = ['Date', 'sp500']

# Convert 'Date' columns to datetime and then extract only the date part
stocks_df['Date'] = pd.to_datetime(stocks_df['Date']).dt.date
SP500['Date'] = pd.to_datetime(SP500['Date']).dt.date

# Merge the dataframes on Date, ensuring you're only using the date (no time)
